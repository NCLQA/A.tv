...
플레이어 > PIP 재생 중 Full player 전환
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    class=android.view.View
    Sleep    1
    ${current_runtime}=    Get Text    com.skt.nugu.apollo.stg:id/tv_end_time
...
플레이어 > 컴포넌트 미노출 상태
    ...
    FOR  ${i}  IN RANGE    0    5    #컴포넌트 정상 노출될때까지 화면 탭 반복
        Wait Until Keyword Succeeds    1 min    3 sec    Click Element    class=android.view.View    
        ${pass}=    Run Keyword And Return Status    Wait Until Page Contains Element    com.skt.nugu.apollo.stg:id/tv_title
        Exit For Loop If    ${pass}  
        Sleep    1
    END  
    Sleep    5
    Page Should Not Contain Element    com.skt.nugu.apollo.stg:id/tv_title
...
플레이어 > 컴포넌트 미노출 상태에서 콘텐츠 전환
    ...
    Sleep    1
    ${right_swipe}    Get Text    com.skt.nugu.apollo.stg:id/tv_title  
    Sleep    5  
    Wait Until Keyword Succeeds    1 min    3 sec    Swipe By Percent    30    50    70    50    #좌측 스와이프
    Sleep    1
    ${left_swipe}    Get Text    com.skt.nugu.apollo.stg:id/tv_title   
...
다운로드 관리 화면 구성 및 제공 문구 개선
    ...
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    com.skt.nugu.apollo.stg:id/textView24    #자동 다운로드 사용
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    com.skt.nugu.apollo.stg:id/textView28    #다운로드 공간 설정
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    com.skt.nugu.apollo.stg:id/textView47    #다운로드 콘텐츠
...
편성표 - 도움말
    ...
    Page Should Not Contain Element    com.skt.nugu.apollo.stg:id/title    #채널 안내 팝업 종료 확인