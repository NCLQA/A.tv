...
45403 채널 정보 확인
    ...
    미디어 메뉴 진입
    Sleep    5
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skt.nugu.apollo.stg:id/controller_container
    Sleep    1
    ${half}    Get Text    id=com.skt.nugu.apollo.stg:id/tv_title    #half player 채널명 저장
    FOR  ${i}  IN RANGE    0    10
        Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skt.nugu.apollo.stg:id/tabBorder
        ${pass}=    Run Keyword And Return Status    Click Element    Xpath=//android.view.ViewGroup[@resource-id="com.skt.nugu.apollo.stg:id/bottom_container"]/android.view.ViewGroup
    ...
    Sleep    2
    ${full}    Get Text    id=com.skt.nugu.apollo.stg:id/tv_title    #full player 채널명 저장
    Sleep    1
...
45435 플레이어 컴포넌트 > 속도 조절 버튼 노출 확인
    ...
    FOR  ${i}  IN RANGE    0    10
        Wait Until Keyword Succeeds    1 min    3 sec    Click Element    class=android.view.View
        ${more_contain}=    Run Keyword And Return Status    Click Element    com.skt.nugu.apollo.stg:id/iv_more
...
45457 플레이어 > 팝업 플레이어(PIP) 전환 > 에이닷 재진입
    ...
    Sleep    5
    Wait Until Keyword Succeeds    1 min    3 sec    Wait Until Element Is Visible    com.skt.nugu.apollo.stg:id/cl_top_pannel
...
45553 화면 내 팝업이 노출되는 상황에서 콘텐츠 전환
    ...
    FOR  ${i}  IN RANGE    0    10    #상세정보 버튼 인식 시점까지 컴포넌트 호출 반복
        ...
        Sleep    5
    END
    Wait Until Element Is Visible    com.skt.nugu.apollo.stg:id/player_panel
...
59088 선호도 선택 완료
    ...
    FOR  ${i}  IN RANGE    0    5    #좋아요 버튼 선택 시점까지 컴포넌트 호출 반복
        Wait Until Keyword Succeeds    1 min    3 sec    Click Element    class=android.view.View
        ${info_contain}=    Run Keyword And Return Status    Click Element    com.skt.nugu.apollo.stg:id/iv_like
        Exit For Loop If    ${info_contain}
        Sleep    1
    END
    Sleep    2
    ...
    FOR  ${i}  IN RANGE    0    5    #싫어요 버튼 선택 시점까지 컴포넌트 호출 반복
        Wait Until Keyword Succeeds    1 min    3 sec    Click Element    class=android.view.View
        ${info_contain}=    Run Keyword And Return Status    Click Element    com.skt.nugu.apollo.stg:id/iv_disLike
        Exit For Loop If    ${info_contain}
        Sleep    1
    END
    Sleep    2
...
87143 프로골프 공통 General
    #1. A. 프로골프 홈 진입
    #2. 상단 영역, 중단 영역 및 하단 영역 확인

    [Tags]    프로골프
    골프 메뉴 진입
    Sleep    3
    Wait Until Keyword Succeeds    1 min    3 sec    Element Should Be Visible    com.skt.nugu.apollo.stg:id/appbar    #Top bar 영역
    Wait Until Keyword Succeeds    1 min    3 sec    Element Should Be Visible    com.skt.nugu.apollo.stg:id/tab_home    #세부 메뉴 영역
    Wait Until Keyword Succeeds    1 min    3 sec    Element Should Be Visible    Xpath=//android.view.View[@content-desc="에이닷 로고"]    #Bottom bar 영역 에이닷 로고
    Sleep    1

87178 프로골프 골프 피드 : 대회 일정 카드 - 대회 일정 더보기
    #0. 프로골프 대회 진행 중 or 대회 예정 상태
    #1. A. 프로골프 홈 진입 > 피드 탭 > 대회 일정 카드 확인
    #2. 대회 일정 더보기 버튼 선택
    #3. 대회 일정탭으로 진입 확인 시 PASS

    [Tags]    프로골프
    골프 메뉴 진입
    Sleep    3
    FOR  ${i}  IN RANGE    0    10    #대회 일정 더보기 노출시점까지 스크롤
        Swipe By Percent    30    60    20    10
        ${pass}=    Run Keyword And Return Status    Wait Until Page Contains Element    com.skt.nugu.apollo.stg:id/moreView
        Exit For Loop If    ${pass}
        Sleep    1
    END
    Sleep    3
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skt.nugu.apollo.stg:id/moreView
    ${대회일정}    Get Text    Xpath=//android.widget.TextView[@text="대회일정"]
    Should Match Regexp    ${대회일정}    ^대회일정    #대회 일정 탭 노출 되었는지 확인
    Sleep    1

87757 선수 AI 하이라이트 : 선호도
    #1. A. 프로골프 홈 진입 > 종합 순위 탭 > 리그 칩 > 순위 영역 > 임의의 선수명 선택 > AI 하이라이트 영상 선택
    #2. 선호도 버튼 확인

    [Tags]    프로골프
    골프 메뉴 진입
    Sleep    3
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.LinearLayout[@content-desc="종합순위"]
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    Xpath=//android.widget.TextView[@resource-id="com.skt.nugu.apollo.stg:id/tv_prefix" and @text="1"]
    Sleep    1
    FOR  ${i}  IN RANGE    0    10    #대회 일정 더보기 노출시점까지 스크롤
        Swipe By Percent    30    60    20    10
        ${pass}=    Run Keyword And Return Status    Wait Until Page Contains Element    Xpath=//android.widget.TextView[@text="AI 하이라이트"]
        Exit For Loop If    ${pass}
        Sleep    1
    END
    Sleep    1
    Wait Until Keyword Succeeds    1 min    3 sec    Click Element    com.skt.nugu.apollo.stg:id/rv_highlights
    Wait Until Keyword Succeeds    1 min    3 sec    Element Should Be Visible    com.skt.nugu.apollo.stg:id/cl_like
    Wait Until Keyword Succeeds    1 min    3 sec    Element Should Be Visible    com.skt.nugu.apollo.stg:id/cl_disLike
    Sleep    1
